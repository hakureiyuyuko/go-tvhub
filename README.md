...
- **运行状态**：管理页实时显示每路的**源站投递速率**（`1.00x` = 源站按实时给流；`0.48x` = 源站只给了一半），小于 0.9x 时播放页会自动弹告警条——把「源站限速」和「面板/网络问题」一眼分开。
...
| `-set` | 空 | 写入设置项后退出，格式 `key=value`，多项用逗号分隔，键名见「参数设置」页。例：`tvhub -data /var/lib/tvhub -set ts_fix=0,hls_time=2` |
...
| 时间戳修复 | 关闭 | 用数据到达时间重建单调时间轴，治源站时间戳跳跃/倒退导致的分片时长混乱。**前提是源站按实时投递**：源站限速时会把整路拉成慢放，所以默认关闭，只在确认源站时间戳坏掉、且源站投递正常时才开（详见下面常见问题） |
| 强制按分片时长切片 | 关闭 | 不等关键帧、到点就切。源站关键帧间隔很长，或开启时间戳修复后分片仍然偏大（4-8 秒）时打开 |
...
| 探测间隔（毫秒） | 1500 | 两次探测之间等多久。IPTV 平台按账号限流，被限流过一次就调到 5000-8000 更保险（代价是跑得慢） |
...
- **一键探测全部**：服务端后台任务，**逐个**调 ffprobe（串行，每个间隔 1.5 秒），进度实时回传，可以关掉页面去忙别的；只探测当前**启用中**的频道。
- ☑ **自动停用探测失败的频道**（默认勾选）：探测失败的频道会被自动停用，用户端立刻看不到；随时可以在列表里点「启用」恢复。
- **停用已标记失败的**：如果你第一次只想看看结果（取消勾选），之后再点这个按钮，按数据库里已有的探测结果批量停用，不用重新探测。
- ☐ **包含已停用的频道（复查旧台）**：勾上后连已停用的也一起探，用于过一段时间复查「之前不能看的台现在能不能看了」（套餐变动、免费开放期等）。注意这一样消耗额度，所以只在需要时才开。
- 探测结果（编码/分辨率/帧率/音频/码率）会写进数据库，列表里直接能看到，换机器/重启都不会丢。
- 中途可点「取消」，已在跑的几条会自然结束。

> ⚠️ **探测消耗的是你账号的额度，跟机顶盒用的是同一套鉴权。**
>
> 实测教训：短时间内跑了几轮全量探测（约 250 次会话/40 分钟）之后，IPTV 平台直接返回
> `429 RateLimitedExceeded: please try again in 1 hour` —— **不只是面板，家里机顶盒也一起看不了了**，只能等一小时。
>
> 所以：并发已经硬性改成 **1（串行）+ 每个间隔 1.5 秒**（把频率压到接近“人手动换台”的水平），
> 149 个频道大约 10-20 分钟。**这个按钮一辈子只需要点一次**（探完导出干净列表即可），
> 平时只想复查某个台，就点那一行的「探测」（单个探测 = 机顶盒换一次台，很轻）。
...
**画面一直卡，调缓冲/网络都没用** — 看一眼服务端的分片文件：如果 `#EXT-X-TARGETDURATION` 变成十几秒、`#EXTINF` 里 1 秒和 10 秒混在一起，那是**源站的时间戳在倒退**，不是缓冲问题：

```bash
# 正常应该是接近「分片时长」的稳定值
cat /var/lib/tvhub/hls/*/index.m3u8
```

某个 IPTV 源实测的 ffmpeg 日志：

```
[vost#0:0/copy] Non-monotonic DTS; previous: 315000, current: 271800; changing to 315001
[in#0/rtsp] RTP: PT=21: bad cseq 0e20 expected=ef10
[in#0/rtsp] Packet corrupt (stream = 0)
```

时间戳往后跳 → ffmpeg 被迫“修补” → 分片时长乱掉 → 播放器时间轴跟着乱 → 一直卡。走 TCP 也照样出现，因为**源站发出来就是这样**（RTP 序列号跳号），不是传输丢包。

解法一（首选）：源站时间戳坏掉、但源站投递正常时，打开「参数设置 → 时间戳修复」。它用数据到达时间重建单调时间轴，实测分片立刻从 `1.0 / 11.0` 变成稳定的 `2.0~2.6` 秒。

注意它**不能滥用**：这个开关假设「数据什么时候到 ≈ 媒体时间到哪儿」。源站一旦限速（只给 0.3-0.5x 实时），到达时间就不再等于媒体时间，整路会变成慢放；这种情况按下一节处理。所以面板默认关闭，只在确认时间戳坏了才开。

如果开启后分片仍然偏大（比如稳定在 4-8 秒、延迟很高），那是源站的**关键帧间隔本来就很长**（`-c:v copy` 只能在关键帧处切片）。先确认一下真实 GOP：

```bash
# 数一下关键帧之间隔了多少帧（25fps 时 50 帧 = 2 秒）
ffprobe -select_streams v:0 -show_entries packet=flags -of csv=p=0 sample.ts | awk '/K/{print NR-p}{p=NR}'
```

GOP 确实很长的话，把「强制按分片时长切片」打开，分片会被压回你设定的秒数（代价是分片可能从非关键帧开始，刚进入频道时可能有 1-2 秒花屏）。

**画面像被慢放了，看着看着就不动了** — 先看「管理 → 运行状态 → 源站投递」这一列：

- `1.00x` 左右：源站投递正常，问题在别处（换台、重启转发再看）。
- 明显小于 1（实测遇到过 `0.48x`）：**源站没按实时给数据**，也就是数据本身就没到。这时播放端只有两种下场：整路变慢（开着「时间戳修复」时特别明显，连声音都变得低沉），或者按原速播但缓冲被抽干、看着看着定住不动。面板只是如实转发，补不出没到的数据。

旁证（都不用去碰源站）：
- 「源站投递」旁边的「帧率 x%」= 当前帧率相对本会话峰值；这一项与时间戳设置无关，可以交叉验证（开了「时间戳修复」时速率会恒显示 `1.00x`，这时就看帧率百分比）。
- 数一下 2 秒分片里的音频包：AAC 正常是 90 多个（48kHz、每包 1024 采样），只剩 40 多个就说明源站少给了一半。
- 想手工核对也可以，在服务器上直接数最新分片的音频包：`ffprobe -v error -count_packets -select_streams a -show_entries stream=nb_read_packets -of csv=p=0 /var/lib/tvhub/hls/ch1-*/seg_00001.ts`（文件名换成最新的分片）。

处理顺序：
1. 换个频道试试。**多个频道一起慢**就是账号/链路级限速：IPTV 平台按账号共享额度，高峰期限速很常见。
2. 拿家里机顶盒/手机 IPTV 看同一个直播台对照：如果它也一样慢/一样卡，那就是源站侧，等一会儿或换个时间段再看，不用去查面板和网络。
3. 不要在这个窗口里反复跑「一键探测」、反复重启转发——那都是在同一个账号额度上加码，可能把限速窗口续期。

播放页检测到投递不足会直接在画面下方挂一条告警，写明倍率和「不是面板的问题」——看到它就不用再去排查浏览器和本地网络了。

**探测一通之后，机顶盒也不能看了，提示 `429 RateLimitedExceeded: please try again in 1 hour`** — 账号被 IPTV 平台限流了。这是**账号级**的限制（探测用的 AuthInfo 与机顶盒相同），所以面板和机顶盒会一起中招。

做法：**等**（报错里写着一小时），期间不要反复重试、也不要反复重启光猫/路由器（可能在给限流窗口续期）。普通上网不受影响。恢复后别再连着跑全量探测；面板里也不要快速连点换台（那也是同一个额度）。

**所有频道都连不上并提示 `No route to host`** — 同样是被限流/封禁的另一种表现（第一版并发 6 那一轮之后出现过）。处理方式同上：等，别重试。
