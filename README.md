...
| 时间戳修复 | 开启 | 用数据到达时间重建单调时间轴，治源站时间戳跳跃/倒退导致的分片时长混乱（详见下面常见问题） |
...
解法：「参数设置 → 时间戳修复」保持开启（默认开）。它用数据到达时间重建单调时间轴，实测分片立刻从 `1.0 / 11.0` 变成稳定的 `2.0~2.6` 秒。只有源站本身就是突发式传输（一小段时间内把好几秒数据灌过来）时才建议关掉。
